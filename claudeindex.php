...
// ============================================================
//   LOCAL PDF ENGINE AUTO-AUTHENTICATION & MULTI-FORM ROUTING
// ============================================================
if ($_SERVER['REMOTE_ADDR'] === '127.0.0.1' || $_SERVER['REMOTE_ADDR'] === '::1') {
    $_SESSION['authenticated'] = true;
    
    // Capture the query parameters passed from Gen_pdf_vr.php
    $op        = isset($_GET['VR_NO']) ? strtoupper(trim($_GET['VR_NO'])) : '';
    $form_type = isset($_GET['FORM_TYPE']) ? trim($_GET['FORM_TYPE']) : 'declaration';
    
    if ($op) {
        // Fallback array lookup configuration
        if (isset($FALLBACK_DATA[$op])) {
            $result = $FALLBACK_DATA[$op];
            $_SESSION['last_result'] = $result;
            
            // Route dynamically based on the incoming form layout flag
            if ($form_type === 'referral') {
                $view = 'referral_form';
            } else {
                $view = 'declaration_form';
            }
        } 
        // Database live query fallback lookup configurations
        elseif ($db_live && $pdo) {
            $sql = "
                SELECT
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'generate_forms' && isset($_SESSION['authenticated'])) {
        ...
        if ($result) {
            ...
            // Define clean Windows workspace locations
            $op_safe  = preg_replace('/[^A-Z0-9\-]/', '', strtoupper($result['op_number'] ?? 'OP'));
            
            // Set up absolute paths
            $script   = __DIR__ . '\\generate_forms.js';
            $outFile  = __DIR__ . '\\ICF_Hearing_Aid_Forms.docx';
            $tmpJson  = __DIR__ . '\\tmp_payload_' . $op_safe . '.json';

            // Clean any old files lingering in the workspace
            if (file_exists($outFile)) { @unlink($outFile); }
            
            // Save the dynamic patient data safely to a physical file first
            file_put_contents($tmpJson, $payload);

            // Navigate directly to the directory and execute node locally
            shell_exec("cd /d C:\\xampp\\htdocs\\hearing-portal && \"C:\\Program Files\\nodejs\\node.exe\" generate_forms.js " . escapeshellarg($tmpJson));

            // Clean up the temporary JSON payload file immediately
            if (file_exists($tmpJson)) { @unlink($tmpJson); }

            if (file_exists($outFile)) {
                // Serve download instantly to browser
                ...
                // Clean up workspace document
                @unlink($outFile);
                exit;
            } else {
                $form_status = 'error';
                $error = 'Form generation failed. Please check that generate_forms.js is saved inside C:\xampp\htdocs\hearing-portal\.';
            }       
        } else {
            $view = 'search';
        }
...
<?php elseif ($view === 'declaration_form' && $result): ?>
<main style="max-width:860px;margin:40px auto 60px;padding:40px;background:#fff;border:1px solid #e2e8f0;border-radius:14px;" class="fade-in">
    <div style="font-family: 'Inter', sans-serif; line-height: 1.6; color: #1e293b;">
        <h2 style="text-align: center; font-size: 24px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #1e293b; margin-bottom: 5px;">Patient Declaration Form</h2>
        <p style="text-align: center; font-size: 13px; color: #64748b; margin-bottom: 25px;">Occupational Health Management System</p>
        ...
        <table style="width: 100%; margin-bottom: 30px; border-collapse: collapse; font-size: 15px;">
            <tr>
                <td style="padding: 8px 0; width: 180px; color: #64748b; font-weight: 500;">Patient Name</td>
                <td style="padding: 8px 0; font-weight: 600; color: #1e293b;">: <?php echo safe($result['full_name']); ?></td>
            </tr>
            <tr>
                <td style="padding: 8px 0; color: #64748b; font-weight: 500;">OP Number</td>
                <td style="padding: 8px 0; font-weight: 600; color: #6366f1; font-family: monospace;">: <?php echo safe($result['op_number']); ?></td>
            </tr>
            <tr>
                <td style="padding: 8px 0; color: #64748b; font-weight: 500;">Department</td>
                <td style="padding: 8px 0; font-weight: 600; color: #1e293b;">: <?php echo safe($result['department']); ?></td>
            </tr>
        </table>
        
        <p style="text-align: justify; font-size: 15px;color: #334155; margin-bottom: 40px; line-height: 1.8;">
            I hereby declare that the occupational audiological medical check profile detailed inside this dossier is accurate. I acknowledge the occupational assessments completed today and explicitly consent to routing this information to the digital signature verification pipeline.
        </p>
        
        <div style="margin-top: 80px; display: flex; justify-content: space-between;">
            <div>
                <p style="font-size: 14px; color: #64748b; margin-bottom: 50px;">Date: <?php echo date('d-m-Y'); ?></p>
            </div>
            <div style="text-align: center;">
                <p style="margin-bottom: 5px; color: #1e293b;">___________________________</p>
                <p style="font-size: 13px; font-weight: 600; color: #475569; text-transform: uppercase;">Patient Signature</p>
            </div>
        </div>
    </div>
</main>

<?php elseif ($view === 'referral_form' && $result): ?>
<main style="max-width:860px;margin:40px auto 60px;padding:40px;background:#fff;border:1px solid #e2e8f0;border-radius:14px;" class="fade-in">
    <div style="font-family: 'Inter', sans-serif; line-height: 1.6; color: #1e293b;">
        <h2 style="text-align: center; font-size: 24px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #0284c7; margin-bottom: 5px;">Clinical Referral Form</h2>
        <p style="text-align: center; font-size: 13px; color: #64748b; margin-bottom: 25px;">Integral Coach Factory (ICF) Medical Center</p>
        <hr style="border: 0; border-top: 1px solid #bae6fd; margin-bottom: 30px;">
        
        <table style="width: 100%; margin-bottom: 30px; border-collapse: collapse; font-size: 15px;">
            <tr>
                <td style="padding: 8px 0; width: 180px; color: #64748b; font-weight: 500;">Referred Patient</td>
                <td style="padding: 8px 0; font-weight: 600; color: #1e293b;">: <?php echo safe($result['full_name']); ?></td>
            </tr>
            <tr>
                <td style="padding: 8px 0; color: #64748b; font-weight: 500;">Employee ID / Pay</td>
                <td style="padding: 8px 0; font-weight: 600; color: #1e293b;">: <?php echo safe($result['employee_id']); ?> / <?php echo safe($result['basic_pay']); ?></td>
            </tr>
            <tr>
                <td style="padding: 8px 0; color: #64748b; font-weight: 500;">Provisional Diagnosis</td>
                <td style="padding: 8px 0; font-weight: 600; color: #ef4444;">: <?php echo safe($result['provisional_diagnosis']); ?></td>
            </tr>
        </table>
        
        <h3 style="font-size: 16px; font-weight: 600; color: #0369a1; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 0.02em;">Clinical Recommendation Details</h3>
        <div style="background: #f0f9ff; padding: 20px; border-left: 4px solid #0284c7; border-radius: 4px; font-size: 14.5px; color: #0369a1; font-weight: 500; margin-bottom: 40px; line-height: 1.6;">
            <?php echo safe($result['recommendation']); ?>
        </div>
        
        <div style="margin-top: 80px; display: flex; justify-content: space-between;">
            <div>
                <p style="font-size: 14px; color: #64748b; margin-bottom: 50px;">Date: <?php echo safe($result['test_date']); ?></p>
            </div>
            <div style="text-align: center;">
                <p style="margin-bottom: 5px; color: #1e293b;">___________________________</p>
                <p style="font-size: 13px; font-weight: 600; color: #475569; text-transform: uppercase;">Examining Audiologist</p>
            </div>
        </div>
    </div>
</main>
...
<main style="max-width:860px;margin:40px auto 60px;padding:0 24px;" class="fade-in">
  ...
  <div class="forms-banner no-print" style="flex-direction: column; align-items: stretch; gap: 16px;">
    <div style="display: flex; align-items: center; justify-content: space-between; gap: 16px;">
      <div class="forms-banner-info">
        ...
        <div>
          <div style="font-size:14px;font-weight:600;color:#0c4a6e;">ICF Hospital Forms &amp; Documents</div>
          <div style="font-size:12px;color:#0369a1;margin-top:2px;">Generate administrative documents or process digital verification vouchers.</div>
        </div>
      </div>
    </div>

    <div style="display: flex; gap: 12px; justify-content: flex-start; border-top: 1px solid #bae6fd; padding-top: 12px;">
      <form method="POST" style="margin:0;">
      ...
      <form method="GET" action="Gen_pdf_vr.php" target="_blank" style="margin:0;">
        <input type="hidden" name="VR_NO" value="<?= safe($result['test_id']) ?>">
        <input type="hidden" name="VR_DT" value="<?= date('d-m-Y', strtotime($result['test_date'])) ?>">
        <input type="hidden" name="SUPCD" value="<?= safe($result['employee_id']) ?>">
        <input type="hidden" name="IBR_DESC" value="<?= safe($result['provisional_diagnosis']) ?>">
        <input type="hidden" name="BILL_DT" value="<?= date('d-m-Y') ?>">
        <input type="hidden" name="IBR_REM" value="<?= safe($result['recommendation']) ?>">
        <input type="hidden" name="IBR_AMT" value="0.00">
        <input type="hidden" name="IBR_NET" value="0.00">
        <input type="hidden" name="IBR_ALL" value="ALL">
        <input type="hidden" name="PAY_CODE" value="DSC">

        <button type="submit" class="btn-sign-voucher">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/>
          </svg>
          Generate &amp; Sign PDF Voucher
        </button>
      </form>
    </div>
  </div>
